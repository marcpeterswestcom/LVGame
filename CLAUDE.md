...
- **Core loop:** Drinks give points + comp cash but raise `drunk` and drain `hyd`; food lowers `drunk`; water restores `hyd`. `drunk > 80` or `hyd < 20` → sick mode (green wash, wobble, score drain, hysteresis exit at 74/26). `drunk = 100` = blackout, `hyd = 0` = collapse, `x >= TOTAL` = win.
...
- **Hazard signatures** (collision switch in `update()`): flyer → `handsFull` 3s (blocks grabs); showgirl → `dazzled` 2.5s (reversed lane input); mascot → $10 photo fee (−60 pts if broke); tourist → shove to random row +8 drunk; robot → classic −40/+12 drunk. All hits reset combo.
...
- **Tuning:** `DRINKS[]`, `FOODS[]`, `HAZARDS[]`, `ZONES[]`, `TOTAL` (30000), `BASE_SPEED`, meter drain rates at top of `update()`.
