...
const DETOUR_SOUVENIRS = {
  ...
  pool: 'Pool VIP', security: 'Smooth Talker',
};
const DETOURS = {
...
function loadSouvenirs() {
  try {
    const saved = JSON.parse(localStorage.getItem('vss-souvenirs') || '[]');
...
function syncSouvenirs() {
  ...
  $('souvenir-line').textContent = names.length ? `NIGHTLIFE SOUVENIRS ${names.length}/5 · ${names.join(' · ')}`
    : 'FIVE NIGHTLIFE SOUVENIRS TO DISCOVER';
}

function buildDetourStops(g) {
  const stops = [['club', 1100], ['chapel', 5600], ['buffet', 9400], ['pool', 15500], ['security', 21500]];
...
function detourEffectText(choice) {
  ...
  if (choice.kicked) values.push('brief stumble');
  return values.join(' · ');
}
...
function chooseDetour(index) {
  ...
  if (outcome.souvenir) {
    ...
    try { localStorage.setItem('vss-souvenirs', JSON.stringify([...souvenirCollection])); } catch (_) {}
    syncSouvenirs();
  }
...
function resumeDetour() {
  ...
  const kicked = game.detour.outcome?.kicked;
  game.detour = null;
